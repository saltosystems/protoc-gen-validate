...
const timestampcmpTpl = `{{ $f := .Field }}{{ $r := .Rules }}
			{{  if $r.Const }}
				if m.hasPaths(paths, "{{ $f.Name }}") && !ts.Equal({{ tsLit $r.Const }}) {
			...
			{{ if $r.Lt }}
				{{ if $r.Gt }}
					{{  if tsGt $r.GetLt $r.GetGt }}
						if m.hasPaths(paths, "{{ $f.Name }}") && (ts.Sub(gt) <= 0 || ts.Sub(lt) >= 0) {
							err := {{ err . "value must be inside range (" (tsStr $r.GetGt) ", " (tsStr $r.GetLt) ")" }}
							if !all { return err }
							errors = append(errors, err)
						}
					{{ else }}
						if m.hasPaths(paths, "{{ $f.Name }}") && ts.Sub(lt) >= 0 && ts.Sub(gt) <= 0 {
				...
				{{ else if $r.Gte }}
					{{  if tsGt $r.GetLt $r.GetGte }}
						if m.hasPaths(paths, "{{ $f.Name }}") && (ts.Sub(gte) < 0 || ts.Sub(lt) >= 0) {
							err := {{ err . "value must be inside range [" (tsStr $r.GetGte) ", " (tsStr $r.GetLt) ")" }}
							if !all { return err }
							errors = append(errors, err)
						}
					{{ else }}
						if m.hasPaths(paths, "{{ $f.Name }}") && ts.Sub(lt) >= 0 && ts.Sub(gte) < 0 {
					...
					{{ end }}
				{{ else }}
					if m.hasPaths(paths, "{{ $f.Name }}") && ts.Sub(lt) >= 0 {
			...
			{{ else if $r.Lte }}
				{{ if $r.Gt }}
					{{  if tsGt $r.GetLte $r.GetGt }}
						if m.hasPaths(paths, "{{ $f.Name }}") && (ts.Sub(gt) <= 0 || ts.Sub(lte) > 0) {
							err := {{ err . "value must be inside range (" (tsStr $r.GetGt) ", " (tsStr $r.GetLte) "]" }}
							if !all { return err }
							errors = append(errors, err)
						}
					{{ else }}
						if m.hasPaths(paths, "{{ $f.Name }}") && ts.Sub(lte) > 0 && ts.Sub(gt) <= 0 {
				...
				{{ else if $r.Gte }}
					{{ if tsGt $r.GetLte $r.GetGte }}
						if m.hasPaths(paths, "{{ $f.Name }}") && (ts.Sub(gte) < 0 || ts.Sub(lte) > 0) {
							err := {{ err . "value must be inside range [" (tsStr $r.GetGte) ", " (tsStr $r.GetLte) "]" }}
							if !all { return err }
							errors = append(errors, err)
						}
					{{ else }}
						if m.hasPaths(paths, "{{ $f.Name }}") && ts.Sub(lte) > 0 && ts.Sub(gte) < 0 {
					...
					{{ end }}
				{{ else }}
					if m.hasPaths(paths, "{{ $f.Name }}") && ts.Sub(lte) > 0 {
				...
				{{ end }}
			{{ else if $r.Gt }}
				if m.hasPaths(paths, "{{ $f.Name }}") && ts.Sub(gt) <= 0 {
					err := {{ err . "value must be greater than " (tsStr $r.GetGt) }}
					if !all { return err }
					errors = append(errors, err)
				}
			{{ else if $r.Gte }}
				if m.hasPaths(paths, "{{ $f.Name }}") && ts.Sub(gte) < 0 {
			...
			{{ else if $r.LtNow }}
				{{ if $r.Within }}
					if m.hasPaths(paths, "{{ $f.Name }}") && (ts.Sub(now) >= 0 || ts.Sub(now.Add(-within)) < 0) {
						err := {{ err . "value must be less than now within " (durStr $r.GetWithin) }}
						if !all { return err }
						errors = append(errors, err)
					}
				{{ else }}
					if m.hasPaths(paths, "{{ $f.Name }}") && ts.Sub(now) >= 0 {
			...
			{{ else if $r.GtNow }}
				...
				{{ end }}
			{{ else if $r.Within }}
				if m.hasPaths(paths, "{{ $f.Name }}") && (ts.Sub(now.Add(within)) >= 0 || ts.Sub(now.Add(-within)) <= 0) {
